...
-- 키의 일부가 변경될 때 키가 아닌 컬럼이 변경되어야 하면 부분적 함수 의존이 있는 것이다.
...
--  키가 아닌 컬럼이 변경될 때 다른 키가 아닌 컬럼이 변경되어야 하면 이행적 종속이 있는 것이다.
...
INSERT INTO my_table35_person (name, school_id)
VALUES ('지성', 4);   -- not ok
DELETE FROM my_table36_school WHERE id = 1; -- not ok
DELETE FROM my_table36_school WHERE id = 3; -- ok
