...
public class Controller30 {
    ...
    public void method5(MyDto26 dto) {
        // country1 또는 country2에 사는 고객명들 출력
...
//        dto2.setKeyword("%" + dto2.getKeyword() + "%");
    ...
    public String method14(MyDto33Employee employee, RedirectAttributes rttr) {
        ...
        // 모델에 추가
        if (rows == 1) {
            rttr.addFlashAttribute("message", "직원 수정 완료");
        } else {
    ...
    public void method15(Integer id, Model model) {
        // 쿼리 파라미터 id, view 포워딩시 보여줄 model
        MyDto34Customers customer = dao.select9(id);

        model.addAttribute("customer", customer);   // view로 포워딩 한 후 꺼내 쓸 수 있도록
        //  attributeName 으로 사용 한다.
    }

    // POST /main30/sub16
    // 고객 정보 수정 -> /main30/sub15?id=3 로 redirect
    @PostMapping("sub16")
    public String method16(MyDto34Customers customer, RedirectAttributes rttr) {
        //넘어온 정보들 customer로 받기 (자바 빈)
        ...
        if (rows == 1) {
            ...
            rttr.addFlashAttribute("message", "고객 정보 수정에러");
        }

        //리턴하면서 쿼리에 붙여주기 ?id={getId}
